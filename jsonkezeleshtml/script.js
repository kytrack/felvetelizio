let jsontomb = [
  ...
  function Listaz() {
    ...
    for (let i = 0; i < jsontomb.length; i++) {
        ...
        let osszesPont = matekPont + magyarPont;

        if (osszesPont > osszpontszam) {
            tartalom += "<tr>";
            tartalom += "<td>" + jsontomb[i].OM_Azonosito + "</td>";
            tartalom += "<td>" + jsontomb[i].Neve + "</td>";
            tartalom += "<td>" + matekPont + "</td>";
...
document.getElementById("rendezesnev").addEventListener("click", event => {
    if (event.target.textContent=="↓") {
        jsontomb.sort(function(a, b) {
            return a.Neve.localeCompare(b.Neve);
        });
        event.target.textContent="↑"    
    }
    else if(event.target.textContent=="↑"){
        jsontomb.sort(function(a, b) {
            return b.Neve.localeCompare(a.Neve);
        });
        event.target.textContent="↓"
    }
...
document.getElementById("rendezesmatek").addEventListener("click", event => {
    if (event.target.textContent=="↓") {
        jsontomb.sort(function(a, b) {
...
document.getElementById("rendezesmagyar").addEventListener("click", event => {
    if (event.target.textContent=="↓") {
        jsontomb.sort(function(a, b) {
...
document.getElementById("rendezesosszesen").addEventListener("click", event => {
    if (event.target.textContent=="↓") {
        jsontomb.sort(function(a, b) {
...
function ValtTema() {
  ...
  if (switchContainer.classList.contains("on")) {
      ...
      switchElem.style.transform = "translateX(100%)";
