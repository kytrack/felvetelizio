...
function searchOM() {
    ...
    var omPattern = /^\d{11}$/;
    if (!omPattern.test(omInput)) {
        errorDiv.innerHTML = 'Hibás OM azonosító! Kérlek adj meg pontosan 11 számjegyet.';
        resultTable.style.display = 'none';
        return;
    }

    var searchData = testdata.filter(function (item) {
        return item.OM_Azonosito === omInput;
    });
    ...
    if (searchData.length > 0) {
        searchData.forEach(function (item) {
            ...
            cell3.innerHTML = item.ErtesitesiCime;
            cell4.innerHTML = item.Email;
            cell5.innerHTML = item.SzuletesiDatum;
            cell6.innerHTML = item.Matematika;
    ...
    var tobbpont = []
    var searchData2 = testdata.filter(function (item) {
        return item.Magyar + item.Matematika > osszpont;
    });

    if (searchData2.length > 0) {
        searchData2.forEach(function (item) {
            var row = resultBody2.insertRow();
            var cell1 = row.insertCell(0);
            var cell2 = row.insertCell(1);
            cell1.innerHTML = item.OM_Azonosito;
            cell2.innerHTML = item.Neve;
